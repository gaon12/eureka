...
router.get("/auth", isSignin, async (req, res) => {
  try {
    ...
    const [users] = await db.query(
      "SELECT isAdmin FROM user WHERE dong = ? AND ho = ?",
      [dong, ho],
    );
...
router.get("/info", isAdmin, async (req, res, next) => {
  try {
    const [users] = await db.query(
      "SELECT dong, ho, username, movein, phone1, phone2 FROM user WHERE isAdmin != 1",
    );
  ...
  try {
    ...
    if (dong && ho && pw) {
      ...
      const [existUser] = await db.query(
        "SELECT * FROM user WHERE dong = ? AND ho = ?",
        [dong, ho],
      );
  ...
  try {
    ...
    if (dong && ho && username && pw1 && pw2 && phone1 && movein) {
      ...
      if (existUser[0].length <= 0 && pw1 === pw2) {
        ...
        if (phone2) {
          await db.query(
            "INSERT INTO user (dong, ho, username, pw, phone1, phone2, movein) VALUES(?, ?, ?, ?, ?, ?, ?)",
            [dong, ho, username, hasedPw, phone1, phone2, movein],
          );
        } else {
          await db.query(
            "INSERT INTO user (dong, ho, username, pw, phone1, movein) VALUES(?, ?, ?, ?, ?, ?)",
            [dong, ho, username, hasedPw, phone1, movein],
          );
        }
